...
class uart_daq:
    ...
    def _uart_recv(self, serial_buffer_read):
    ...
    def _format_frame(self, buffer_frame, imagearray):
        ...
        while 1:
            if(buffer_frame.qsize() > 0): # for safety
                ...
                imagearray_copy = np.zeros(int(self.num_pixel_assert), np.uint8)
                ...
                if len(pixel_vector) > self.num_pixel_assert:
                    pixel_vector = pixel_vector[0:self.num_pixel_assert]
    ...
    def capture(self):
        ...
        imagearray.put(np.zeros(int(self.num_pixel_assert), np.uint8))

        p_uart_recv = multiprocessing.Process(target=self._uart_recv, args=(serial_buffer_read, ))
