...
@pytest.mark.parametrize("test_input,header_length_words,preamble_length_words,reverse_header_bits,reverse_header_bytes,reverse_payload_bits,reverse_payload_bytes,expected_header,expected_payload",
    [
        ...
        np.array([0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xAA, 0xBB], dtype=np.uint8)),
        (b'\x12\x34\x56\x78\x00\x11\x22\x33\x44\x55\x66\x77\x88\x99\xAA\xBB',
        1,
        0,
        True,
        ...
        np.array([0x00, 0x88, 0x44, 0xCC, 0x22, 0xAA, 0x66, 0xEE, 0x11, 0x99, 0x55, 0xDD], dtype=np.uint8)),
    ],
    )
...
@pytest.mark.parametrize("input_array,expected_output", [
    (np.array([0x12345678], dtype=np.uint32),
     np.array([0x78563412], dtype=np.uint32)),
    (np.array([0xABCD4574], dtype=np.uint32),
     np.array([0x7445CDAB], dtype=np.uint32)),
    (np.array([0x11002200], dtype=np.uint32),
     np.array([0x00220011], dtype=np.uint32)),
])
